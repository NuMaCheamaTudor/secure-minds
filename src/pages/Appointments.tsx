    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-primary/5 via-background to-secondary/10 p-4">
      <div className="w-full max-w-2xl animate-fade-in">
        ...
        <Card className="p-6 mb-6">
          <p className="mb-4">Here you can see and manage your appointments.</p>
          <div className="space-y-4">
            {sampleAppointments.map(app => (
              <div key={app.id} className="p-4 bg-muted rounded-lg flex flex-col gap-1">
                <div className="font-semibold">{app.type} with {app.doctor}</div>
                <div>Date: {app.date} at {app.time}</div>
                <div>Location: {app.location}</div>
                <Button variant="outline" size="sm" className="mt-2 self-end">Cancel</Button>
              </div>
            ))}
          </div>
        </Card>

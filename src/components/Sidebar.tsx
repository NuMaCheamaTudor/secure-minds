	...
	if (effectiveRole === "doctor") {
		...
		sidebarContent = (
			<>
				...
				<nav className="flex flex-col gap-2 flex-1">
					{navItems.map(item => (
						item.path === "/appointments" ? (
							<Button
								key={item.path}
								className={`py-2 px-4 rounded-lg transition-colors font-medium w-full text-left ${location.pathname === item.path ? "bg-primary text-white" : "hover:bg-muted"}`}
				...
				{showCalendar && (
					<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
						<div className="bg-white rounded-lg p-6 shadow-lg flex flex-col items-center">
							<h3 className="mb-4 text-lg font-semibold">Selectează o dată pentru programare</h3>
							<DatePicker
							...
							<div className="flex gap-2 mt-4">
								<Button onClick={() => setShowCalendar(false)} variant="outline">Închide</Button>
								<Button
									onClick={() => {
										setShowCalendar(false);
										if (selectedDate) {
											alert(`Ai selectat data: ${selectedDate.toLocaleDateString()}`);
											// Poți adăuga aici logica de programare
										}
									}}
				...
				{/* Afișează data selectată sub butonul de programări */}
				{selectedDate && (
					<div className="mt-2 text-sm text-primary font-medium text-center">
						Data selectată: {selectedDate.toLocaleDateString()}
					</div>
				)}
			</>
		);
